...
if __name__=='main':
    ars, = _parse_args()
    def = pd.read_csv(os.path.join(args.filepath, args.filename)
    df = df.replace(regex=r'\.', value='_')
    df = df.replace(regex=r'\_$', value='')
                      
    #add two new features by transforming pday and job features
    ...
    #  Randomly sort the data then split out first 70%, second 20%, and last 10% to train, test and val sets respectively
